  ...
  class ReviewWidget extends ot {
    static styles = r`
        :host {
          ...
          --w-foreground: 0 0% 98%;
          --w-card: 280 10% 15%; /* Dark Purple-Gray */
        ...
        :host([theme="light"]) {
            --w-background: 220 20% 98%;
            --w-foreground: 280 10% 15%;
            --w-card: 220 20% 100%;
            --w-card-foreground: 280 10% 15%;
            --w-primary: 283 60% 30%;
            --w-primary-foreground: 0 0% 98%;
            --w-secondary: 220 20% 95%;
            --w-muted: 220 20% 95%;
            --w-muted-foreground: 280 10% 40%;
            --w-accent: 308 100% 43%;
            --w-accent-foreground: 0 0% 98%;
            --w-border: 220 20% 90%;
            --w-input: 220 20% 90%;
        ...
        .form-input, .form-textarea { display: flex; width: 100%; border-radius: calc(var(--w-radius) - 4px); border: 1px solid hsl(var(--w-input)); background-color: hsl(var(--w-background)); padding: 0.5rem 0.75rem; font-size:0.875rem; box-sizing: border-box; color: hsl(var(--w-foreground)); }
    ...
    async fetchReviews() {
      ...
      try {
        ...
        if (this.widget.theme && this.widget.theme === 'light') {
            this.theme = 'light';
        }
      } catch (error) {
    ...
    render() {
      ...
      const allReviewsSorted = totalReviews > 0 ? [...this.widget.reviews].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()) : [];
            ...
            ${totalReviews > 0 ? J`
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="card md:col-span-1 flex flex-col items-center justify-center text-center p-6">
                  ...
                  ${this._renderStarRating(this.overallRating)}
              ...
              <div>
                ...
                ${totalReviews > 0 ? this._renderReviewGrid(allReviewsSorted) : J`
                    ...
                    <div class="rating-stars">
                        ${[1, 2, 3, 4, 5].map(star => J`
                            <button type="button" 
                                ...
                                class="star-btn">
                                <svg class="${(this.hoverRating >= star || this.rating >= star) ? 'star-filled' : 'star-empty'}" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                            </button>
                        `)}
                    </div>
                </div>
